...
function OverviewView({ token, days, onUnauthorized }: { token: string; days: number; onUnauthorized: () => void }) {
  const [data, setData] = useState<AdminOverview | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const load = useCallback(async () => {
    setLoading(true); setError('');
    try { setData(await adminApi.overview(token, days)); }
    catch (reason) { if (reason instanceof AdminApiError && reason.status === 401) onUnauthorized(); else setError(reason instanceof Error ? reason.message : '加载失败。'); }
    finally { setLoading(false); }
  }, [days, onUnauthorized, token]);
  useEffect(() => { void load(); }, [load]);
  if (loading || error || !data) return <SectionState loading={loading} error={error} retry={load} />;
...
function ReportsView({ token, onUnauthorized }: { token: string; onUnauthorized: () => void }) {
  ...
  const [query, setQuery] = useState(''); const [risk, setRisk] = useState(''); const [source, setSource] = useState(''); const [selected, setSelected] = useState<AdminReport | null>(null);
  const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const load = useCallback(async () => { setLoading(true); setError(''); const params = new URLSearchParams({ page: String(page), page_size: '20' }); if (query) params.set('query', query); if (risk) params.set('risk_level', risk); if (source) params.set('source', source); try { const result = await adminApi.reports(token, params); setItems(result.items); setTotal(result.total); } catch (reason) { if (reason instanceof AdminApiError && reason.status === 401) onUnauthorized(); else setError(reason instanceof Error ? reason.message : '加载失败。'); } finally { setLoading(false); } }, [onUnauthorized, page, query, risk, source, token]);
  useEffect(() => { void load(); }, [load]);
  return <div className="space-y-4"><div className="flex flex-wrap gap-3"><label className="relative min-w-64 flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-gray-400" /><input value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} placeholder="搜索报告 ID、公司或岗位" className="h-10 w-full rounded-md border border-gray-300 pl-9 pr-3 text-sm outline-none focus:border-gray-600" /></label><select value={risk} onChange={event => { setRisk(event.target.value); setPage(1); }} className="h-10 rounded-md border border-gray-300 bg-white px-3 text-sm"><option value="">全部风险</option><option>低</option><option>中</option><option>高</option><option>极高</option></select><select value={source} onChange={event => { setSource(event.target.value); setPage(1); }} className="h-10 rounded-md border border-gray-300 bg-white px-3 text-sm"><option value="">全部来源</option><option value="model">模型</option><option value="fallback">规则降级</option></select></div>
    <section className="rounded-lg border border-gray-200 bg-white">{loading || error ? <SectionState loading={loading} error={error} retry={load} /> : <ReportTable reports={items} onSelect={setSelected} />}</section>
    <div className="flex items-center justify-between text-sm text-gray-500"><span>共 {total} 条</span><div className="flex items-center gap-2"><button aria-label="上一页" disabled={page === 1} onClick={() => setPage(value => value - 1)} className="rounded-md border border-gray-300 p-2 disabled:opacity-40"><ChevronLeft className="h-4 w-4" /></button><span>第 {page} 页</span><button aria-label="下一页" disabled={page * 20 >= total} onClick={() => setPage(value => value + 1)} className="rounded-md border border-gray-300 p-2 disabled:opacity-40"><ChevronRight className="h-4 w-4" /></button></div></div>
...
function FeedbacksView({ token, onUnauthorized, onChanged }: { token: string; onUnauthorized: () => void; onChanged: () => void }) {
  const [items, setItems] = useState<AdminFeedback[]>([]); const [total, setTotal] = useState(0); const [page, setPage] = useState(1); const [kind, setKind] = useState('all'); const [status, setStatus] = useState('pending'); const [notes, setNotes] = useState<Record<string, string>>({}); const [busy, setBusy] = useState(''); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const load = useCallback(async () => { setLoading(true); setError(''); const params = new URLSearchParams({ page: String(page), page_size: '20', kind, status }); try { const result = await adminApi.feedbacks(token, params); setItems(result.items); setTotal(result.total); setNotes(Object.fromEntries(result.items.map(item => [item.id, item.reviewer_note || '']))); } catch (reason) { if (reason instanceof AdminApiError && reason.status === 401) onUnauthorized(); else setError(reason instanceof Error ? reason.message : '加载失败。'); } finally { setLoading(false); } }, [kind, onUnauthorized, page, status, token]);
  useEffect(() => { void load(); }, [load]);
  const review = async (item: AdminFeedback, nextStatus: string) => { setBusy(item.id); try { await adminApi.reviewFeedback(token, item.kind, item.id, nextStatus, notes[item.id] || ''); await load(); onChanged(); } catch (reason) { if (reason instanceof AdminApiError && reason.status === 401) onUnauthorized(); else setError(reason instanceof Error ? reason.message : '审核失败。'); } finally { setBusy(''); } };
  return <div className="space-y-4"><div className="flex flex-wrap gap-3"><select value={kind} onChange={event => { setKind(event.target.value); setPage(1); }} className="h-10 rounded-md border border-gray-300 bg-white px-3 text-sm"><option value="all">全部反馈</option><option value="report">报告纠错</option><option value="interview">面试反馈</option></select><select value={status} onChange={event => { setStatus(event.target.value); setPage(1); }} className="h-10 rounded-md border border-gray-300 bg-white px-3 text-sm"><option value="pending">待审核</option><option value="approved">已通过</option><option value="rejected">已驳回</option><option value="all">全部状态</option></select><span className="self-center text-sm text-gray-500">{total} 条</span></div>{loading || error ? <SectionState loading={loading} error={error} retry={load} /> : items.length === 0 ? <div className="rounded-lg border border-gray-200 bg-white py-16 text-center text-sm text-gray-500">暂无反馈</div> : <div className="space-y-3">{items.map(item => <article key={`${item.kind}-${item.id}`} className="rounded-lg border border-gray-200 bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold text-gray-900">{item.title}</span><span className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">{item.kind === 'report' ? '报告纠错' : '面试反馈'}</span><span className={`rounded px-2 py-1 text-xs ${item.review_status === 'approved' ? 'bg-emerald-100 text-emerald-700' : item.review_status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{statusLabel(item.review_status)}</span></div><p className="mt-1 text-xs text-gray-500">{item.company_name || '未知公司'} · {item.job_title || item.report_id || '未关联报告'} · {formatDate(item.created_at)}</p></div>{item.risk_level && <span className={`rounded px-2 py-1 text-xs font-medium ${riskClass(item.risk_level)}`}>{item.risk_level} · {item.overall_score}</span>}</div><p className="mt-4 whitespace-pre-wrap text-sm leading-6 text-gray-700">{item.content}</p>{item.tags.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{item.tags.map(tag => <span key={tag} className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">{tag}</span>)}</div>}<div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]"><input value={notes[item.id] || ''} onChange={event => setNotes(value => ({ ...value, [item.id]: event.target.value }))} maxLength={500} placeholder="内部审核备注" className="h-10 rounded-md border border-gray-300 px-3 text-sm outline-none focus:border-gray-600" /><div className="flex gap-2"><button disabled={busy === item.id} onClick={() => void review(item, 'approved')} className="flex h-10 items-center gap-1 rounded-md bg-emerald-600 px-3 text-sm font-medium text-white hover:bg-emerald-700 disabled:opacity-50"><Check className="h-4 w-4" />通过</button><button disabled={busy === item.id} onClick={() => void review(item, 'rejected')} className="flex h-10 items-center gap-1 rounded-md bg-red-600 px-3 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-50"><ShieldX className="h-4 w-4" />驳回</button>{item.review_status !== 'pending' && <button disabled={busy === item.id} onClick={() => void review(item, 'pending')} className="h-10 rounded-md border border-gray-300 px-3 text-sm hover:bg-gray-50">待审</button>}</div></div></article>)}</div>}<div className="flex justify-end gap-2"><button aria-label="上一页" disabled={page === 1} onClick={() => setPage(value => value - 1)} className="rounded-md border border-gray-300 p-2 disabled:opacity-40"><ChevronLeft className="h-4 w-4" /></button><span className="self-center text-sm text-gray-500">第 {page} 页</span><button aria-label="下一页" disabled={page * 20 >= total} onClick={() => setPage(value => value + 1)} className="rounded-md border border-gray-300 p-2 disabled:opacity-40"><ChevronRight className="h-4 w-4" /></button></div></div>;
}
...
function SecurityView({ token, days, onUnauthorized }: { token: string; days: number; onUnauthorized: () => void }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { setData(await adminApi.security(token, days)); } catch (reason) { if (reason instanceof AdminApiError && reason.status === 401) onUnauthorized(); else setError(reason instanceof Error ? reason.message : '加载失败。'); } finally { setLoading(false); } }, [days, onUnauthorized, token]); useEffect(() => { void load(); }, [load]);
  if (loading || error || !data) return <SectionState loading={loading} error={error} retry={load} />;
