...
2. For no-cost HTTPS without buying a domain, use an IP-mapped hostname such as
   `SITE_ADDRESS=43-142-15-246.sslip.io` and
   `PUBLIC_ORIGIN=https://43-142-15-246.sslip.io`. Direct IP-only access can use
   `SITE_ADDRESS=:80`, but browsers may upgrade it to unsupported HTTPS.
