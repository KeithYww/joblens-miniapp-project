...
3. **配置项目**
   - **Root Directory**：输入 `web-h5`
   - **Framework Preset**：选择 `Vite`
   - **Build Command**：输入 `npm ci && npm run build`
...
3. **配置服务**
   ...
   - **Build Command**：输入 `npm ci && npm run prisma:generate && npm run build`
...
4. **添加环境变量**
   ...
   - 添加以下变量：
     ...
     - `DATABASE_URL` = Render PostgreSQL 内网连接串
     - `REDIS_URL` = Render Redis 内网连接串
   ...
   - 首次发布及每次 Schema 变更前执行：`npm run prisma:deploy`

   如果数据库是全新空库，直接执行 `npm run prisma:deploy`。如果数据库曾按旧版
   `schema.prisma` 手工建表但没有 Prisma migration 历史，请先备份数据库，然后执行：

   ```bash
   npx prisma migrate resolve --applied 20260713170000_baseline
   npm run prisma:deploy
   ```

   第二条 migration 会新增数据保留字段和索引，并为旧记录回填清理截止时间。
...
2. **添加环境变量**
   ...
   - 添加变量：`VITE_TURNSTILE_SITE_KEY` = 你的 Cloudflare Turnstile Site Key
...
- **数据库与 Redis**：生产环境必须配置 PostgreSQL 与 Redis；服务默认在依赖不可用时拒绝启动，避免静默丢数据或多实例限流失效
...
3. **配置环境变量**
   ...
   DATABASE_URL=postgresql://用户名:密码@数据库地址:5432/joblens
   REDIS_URL=redis://Redis地址:6379
   TURNSTILE_SECRET_KEY=你的Turnstile密钥
