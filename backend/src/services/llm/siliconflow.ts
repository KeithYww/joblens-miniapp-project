...
export class SiliconFlowProvider {
  ...
  constructor(apiKey?: string, model?: string) {
    ...
    this.model = model || process.env.SILICONFLOW_MODEL || 'deepseek-chat';
  }
...
## 输出 JSON 格式：
{
  ...
  "recommendation": string
    ...
    try {
      ...
      const inputTokens = response.data.usage?.prompt_tokens || input.jd_text.length * 2;
      const outputTokens = response.data.usage?.completion_tokens || rawText.length * 2;
      ...
      console.error('SiliconFlow API error:', err);
      throw err;
    }
  }
...
## 输出 JSON 格式：
{
  ...
  "next_questions": string[]
    ...
    try {
      ...
      const latencyMs = Date.now() - startTime;
      const inputTokens = response.data.usage?.prompt_tokens || (input.user_question.length + input.hr_reply.length) * 2;
      const outputTokens = response.data.usage?.completion_tokens || rawText.length * 2;
